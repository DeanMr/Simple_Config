...
- (void)viewDidDisappear:(BOOL)animated
{
    ...
    if ([waitTimerH isValid]) {
        [waitTimerH invalidate];
        waitTimerH = nil;
    }
...
//    
//    NSDictionary *dic = [NSDictionary dictionaryWithObjectsAndKeys:@"any",@"UIApplicationOpenURLOptionsKey", nil];
...
- (void)startWaiting:(NSString *) wait_title :(float)timeout {
    ...
    if (!waitingAlert) {
        ...
        if (timeout>0) {
            waitTimerH = [NSTimer scheduledTimerWithTimeInterval: timeout
                                                         target: self
...
- (void)startWaiting_progress:(NSString *) wait_title :(float)timeout
{
    ...
    if (timeout>0) {
        ...
        waitTimerH = [NSTimer scheduledTimerWithTimeInterval:1.2 target:self selector:@selector(update_progress) userInfo:nil repeats:YES];
        //[[NSRunLoop currentRunLoop]addTimer:waitTimerH forMode:NSDefaultRunLoopMode];
    }
}
